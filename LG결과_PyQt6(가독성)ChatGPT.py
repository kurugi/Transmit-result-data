'''
이 코드는 PyQt6 기반의 GUI 프로그램으로, 병원 결과 파일과 병원 소견 파일의 데이터를 읽어와 LG 전송용 Excel 템플릿으로 변환해 저장하는 기능을 제공합니다.

주요 기능은 다음과 같습니다:

- **파일 선택 및 GUI 구성:**  
  사용자가 병원 결과와 병원 소견 Excel 파일을 선택하고, 진행 상태를 확인할 수 있도록 버튼, 텍스트 출력창, 진행바 등이 구현되어 있습니다.

- **Excel 데이터 처리:**  
  openpyxl 라이브러리를 사용해 파일을 읽고, 특정 행부터 기존 데이터를 삭제한 후 새 데이터를 삽입합니다.

- **데이터 변환 및 매핑:**  
  병원 파일의 각 컬럼과 LG 템플릿의 컬럼을 매핑하고, 주민번호, 진료일자, 계산식 등 필요한 데이터 형식 변환 및 가공을 수행합니다.

- **병원 소견 데이터 추가 매핑:**  
  소견 파일의 데이터와 LG 템플릿의 데이터를 특정 키(예: EMP_NO|SSN)를 기준으로 매칭하여 추가 데이터를 입력합니다.

- **에러 처리 및 피드백:**  
  파일이 없거나 매핑되지 않은 데이터에 대해 오류 및 경고 메시지를 출력해 사용자에게 알려줍니다.

마지막에 PyInstaller 명령어를 통해 실행 파일로 패키징할 수 있도록 주석으로 안내되어 있습니다.


--- 함수별 설명
다음은 각 함수별로 하는 일을 번호로 정리한 설명입니다.

1. **convert_emp_no(value)**  
   - 값이 정수나 실수인 경우, 정수 형태로 변환하여 반환합니다.  
   - 만약 값이 정수로 표현할 수 없으면 문자열로 반환합니다.

2. **convert_to_numeric(value)**  
   - 값이 None, 빈 문자열, 혹은 공백이 아니라면 실수(float)로 변환을 시도합니다.  
   - 변환이 불가능하면 원래의 값을 그대로 반환합니다.

3. **truncate_ssn(value)**  
   - 주민등록번호 문자열이 주어지면 처음 8자리만 잘라서 반환합니다.

4. **convert_mdc_date(value)**  
   - 날짜 문자열에서 하이픈(-)을 제거한 후, 정수로 변환을 시도합니다.  
   - 변환에 실패하면 하이픈이 제거된 문자열을 반환합니다.

5. **extract_sex_no(ssn_value)**  
   - 주민등록번호의 8번째 자리 숫자를 확인하여,  
     - "1", "3", "5"인 경우 22를,  
     - "2", "4", "6"인 경우 21을 반환합니다.  
   - 주민등록번호 형식이 맞지 않으면 None을 반환합니다.

6. **calculate_bm06(bm01_value, sex_no)**  
   - BM01 값을 백분율로 변환 후 제곱하여, 성별 숫자(sex_no)와 곱한 결과를 소수점 한 자리로 반올림해 반환합니다.

7. **MainWindow.\_\_init\_\_()**  
   - PyQt6 기반의 메인 윈도우를 초기화하며,  
   - 파일 경로 변수, 버튼, 텍스트 출력창, 진행바, 상태 라벨 등 UI 요소를 생성 및 배치합니다.  
   - 병원 결과, LG 템플릿, 병원 소견 파일의 컬럼 매핑 정보를 설정합니다.

8. **MainWindow.log(message)**  
   - 전달된 메시지를 GUI의 텍스트 출력창에 추가하여, 진행 상황이나 오류 메시지를 사용자에게 보여줍니다.

9. **MainWindow.select_hospital_file()**  
   - 파일 선택 대화상자를 열어 병원 결과 Excel 파일을 선택하도록 합니다.  
   - 선택된 파일 경로를 저장하고 로그에 기록합니다.

10. **MainWindow.select_opinion_file()**  
    - 파일 선택 대화상자를 통해 병원 소견 Excel 파일을 선택하도록 합니다.  
    - 선택된 파일 경로를 저장하고 로그에 기록합니다.

11. **MainWindow.update_status()**  
    - 병원 결과 파일과 병원 소견 파일의 선택 상태에 따라 상태 라벨의 텍스트를 업데이트하여 사용자에게 현재 진행 상황을 알려줍니다.

12. **MainWindow.map_and_transfer_data()**  
    - 병원 결과 파일과 LG 템플릿 파일을 열어,  
    - LG 템플릿의 4행부터 기존 데이터를 삭제한 후,  
    - 병원 파일의 각 행 데이터를 읽어 컬럼 매핑에 따라 변환 후 LG 템플릿에 입력합니다.  
    - 주민등록번호, 진료일자, BM01 등 필드별로 필요한 변환(예: 데이터 형식 변환, 계산 등)을 적용하고 진행 상황을 진행바로 표시합니다.  
    - 변환된 데이터를 지정한 파일로 저장합니다.

13. **MainWindow.map_matching_rows_to_transformed(wb_lg, ws_lg, lg_headers)**  
    - 병원 소견 파일을 열어,  
    - 소견 파일의 HO_NO와 jumin, 그리고 LG 템플릿의 EMP_NO와 SSN을 결합한 키를 기반으로 매핑합니다.  
    - 두 파일 간에 일치하는 데이터를 찾아서, 해당 행에 소견 데이터를 추가로 입력합니다.  
    - 매칭되지 않는 항목에 대해서는 경고 메시지를 출력하고, 최종적으로 결과 파일에 저장합니다.

14. **MainWindow.run_conversion()**  
    - 필요한 파일(병원 결과, LG 템플릿, 병원 소견)이 모두 선택되었는지 확인합니다.  
    - 저장할 파일 이름을 사용자에게 입력받은 후,  
    - `map_and_transfer_data()`와 `map_matching_rows_to_transformed()` 함수를 차례대로 호출하여 전체 데이터 변환 작업을 수행합니다.  
    - 변환 진행 상황과 완료 메시지를 로그 및 상태 라벨에 출력합니다.
